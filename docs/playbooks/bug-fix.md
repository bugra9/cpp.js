...
- A user reports a runtime error from `crossbind build` or a sample.
...
1. **Reproduce in the smallest sample first.** Don't debug against `examples/mobile-reactnative-cli` if `examples/backend-nodejs-wasm` reproduces the same bug — the smaller the surface, the faster the loop.
...
`ci:linux:build` runs `pnpm run build:examples && pnpm run ci:linux:build:port` — it builds every sample plus zlib as a smoke. `e2e:dev` runs Playwright in dev mode against samples; `e2e:prod` against built artifacts.
...
- [ ] If you touched a public API in `crossbind` exports, search consumers (`grep -rn 'from .crossbind.' plugins examples`) and confirm none break.
...
- **Forgetting transitive plugin impact.** A change in `plugins/rollup` affects `plugins/vite` (which wraps it). Validate both bundler samples.
