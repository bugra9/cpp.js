...
| File | Role |
|------|------|
| `package.json` | + `@cpp.js/plugin-react-native`, `@cpp.js/plugin-react-native-ios-helper` (dependencies), `@cpp.js/plugin-metro` (devDependency), optional `@cpp.js/package-<name>` |
...
| `app.json` / `app.config.{js,ts}` | Add `"@cpp.js/plugin-react-native"` to `plugins` (it ships an Expo config plugin); ensure `prebuild` is configured |
...
# Add "@cpp.js/plugin-react-native" to expo.plugins in app.json before prebuilding
