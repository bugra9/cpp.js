...
export function isCached(depsDir, targets, key, dep, hasIos) {
    if (loadJson(`${depsDir}/.crossbind-rebuild.json`)?.key !== key) return false;
...
function resolveSelector(rebuildOption) {
    const raw = rebuildOption !== undefined ? rebuildOption : process.env.CROSSBIND_REBUILD_DEPS;
...
// Which target dirs a dependency currently ships, so a prebuilt that appears (or is rebuilt)
// after the last configure busts it too. Rebuilt-from-source deps carry a marker key; prebuilt
// ones carry nothing, and without this they used to stay invisible to the stamp - a refreshed
// artifact then never entered the link until the CMake cache was wiped by hand.
function prebuiltFingerprint(dep) {
    const dir = dep.paths?.cmakeDir;
    // No prebuilt at all contributes nothing, so a dependency set with neither markers nor
    // artifacts still stamps as "none".
    if (!dir || !fs.existsSync(dir)) return null;
    const targets = fs.readdirSync(dir, { withFileTypes: true })
        .filter((e) => e.isDirectory())
        .map((e) => {
            const stamp = fs.statSync(`${dir}/${e.name}`).mtimeMs;
            return `${e.name}@${stamp}`;
        })
        .sort();
    return `${dep.general.name}:${targets.join(',')}`;
}

export function computeDependenciesStamp(allDependencies, cacheDir) {
    const keys = allDependencies
        .map((d) => {
            const marker = loadJson(`${cacheDir}/deps/${d.general.name}/.crossbind-rebuild.json`);
            return marker?.key === getOverrideKey(d) ? marker.key : null;
        })
        .filter(Boolean)
        .sort();
    const prebuilts = allDependencies.map(prebuiltFingerprint).filter(Boolean).sort();
    if (keys.length === 0 && prebuilts.length === 0) return 'none';
    return getContentHash([...keys, ...prebuilts].join('|'));
}
