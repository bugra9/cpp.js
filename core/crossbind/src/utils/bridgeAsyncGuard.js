...
// wrap each async registration in #ifdef CROSSBIND_JSPI, and let createLib define
// CROSSBIND_JSPI only for targets whose emccFlags carry -sJSPI.
export default function guardAsyncBindings(bridgeText) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        const line = lines[i];
        const alreadyGuarded = i > 0 && lines[i - 1].trim() === '#ifdef CROSSBIND_JSPI';
        ...
        // an empty statement) stays valid when CROSSBIND_JSPI is undefined.
        const match = /^(\s*)(.*?);?\s*$/.exec(line);
        const indent = match[1];
        const call = match[2];
        const endsStatement = /;\s*$/.test(line);
        out.push('#ifdef CROSSBIND_JSPI');
