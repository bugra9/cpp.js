...
// The Rust producer crate + adapters ship in @crossbind/core-embind-rust. The ENGINE never
...
export default function resolveEmbindRustRoot() {
    ...
    if (!root) {
        ...
        const sibling = path.resolve(here, '../../../embind-rust');
        if (fs.existsSync(`${sibling}/crate/Cargo.toml`)) root = sibling;
    }
    if (!root) {
        throw new Error(`crossbind: Rust bindings need ${PKG} - add it to your (dev)dependencies.`);
    }
    cached = root;
    return root;
}
