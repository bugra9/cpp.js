...
function scanSource(src, acc, { collectTypes, log }) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (enumM) {
            ...
            else log(`crossbind: rust bridge: enum ${enumM[1]} skipped (needs #[repr(i32)] and unit variants)`);
        } else if (structM) {
            ...
            if (isReprC) {
                ...
                else log(`crossbind: rust bridge: struct ${structM[1]} skipped (repr(C) needs derive(Default, Copy) and pub primitive fields)`);
            } else {
...
function parseFn(cls, sig, ctx, log) {
    ...
    for (const p of params) {
        ...
        if (!m || !(known(ty) || ty === JSON_TY || jsTokP || (arcParam && classes.has(arcParam)) || PARAM_ONLY.has(ty) || OPTION_PARAM_RE.test(ty) || isClassRef(ty))) {
            log(`crossbind: rust bridge: ${cls.name}::${name} skipped (unsupported parameter '${p}')`);
            return;
        }
    ...
    if (jsTokR && optional) {
        log(`crossbind: rust bridge: ${cls.name}::${name} skipped (Option<${ret}> returns are not supported - return JsValue::undefined() instead)`);
        return;
    }
    ...
    if (!selfKind) {
        ...
        if (name === 'new') {
            ...
            if (args.length > 3) { log(`crossbind: rust bridge: ${cls.name}::new skipped (max 3 args)`); return; }
            ...
            if (args.length > 2) { log(`crossbind: rust bridge: ${cls.name}::${name} skipped (factories take max 2 args)`); return; }
    ...
    if (retArcInner) {
        ...
        if (optional || throws) {
            log(`crossbind: rust bridge: ${cls.name}::${name} skipped (Arc returns on methods support neither Option nor Result in this wave)`);
            return;
        }
        if (args.length > 4) { log(`crossbind: rust bridge: ${cls.name}::${name} skipped (max 4 args)`); return; }
...
function parseFreeFn(freeFns, sig, ctx, log) {
    ...
    for (const p of rawParams.split(',').map((s) => s.trim()).filter(Boolean)) {
        ...
        const arcParam = ty?.match(ARC_RE)?.[1];
        if (!m || !(known(ty) || ty === JSON_TY || jsTokP || (arcParam && classes.has(arcParam)) || PARAM_ONLY.has(ty) || OPTION_PARAM_RE.test(ty) || isClassRef(ty))) {
            log(`crossbind: rust bridge: fn ${name} skipped (unsupported parameter '${p}')`);
            return;
        }
        args.push({ name: m[1], ty });
    }
    ...
    if (retArc) {
        ...
        if (optional || throws) { log(`crossbind: rust bridge: fn ${name} skipped (Arc returns support neither Option nor Result in this wave)`); return; }
    }
    if (jsTokR && optional) { log(`crossbind: rust bridge: fn ${name} skipped (Option<${ret}> returns are not supported - return JsValue::undefined() instead)`); return; }
    if (optional && !OPTION_INNERS.has(ret)) { log(`crossbind: rust bridge: fn ${name} skipped (Option<${ret}> return is not representable - inners: i32, f64, bool, String)`); return; }
    if (!known(ret) && ret !== JSON_TY && !retArc && !jsTokR) { log(`crossbind: rust bridge: fn ${name} skipped (unsupported return '${ret}')`); return; }
    if (args.length > 4) { log(`crossbind: rust bridge: fn ${name} skipped (max 4 args)`); return; }
...
function emitBridge(model, { userCrate, vectors, log, prelude = '' }) {
    ...
    const wrap = (ty, expr) => (ty === JSON_TY ? `__CrossbindJson(${expr})`
