...
const dep = (name, extra = {}) => ({
    general: { name },
    package: { name: `@crossbind/port-${name}`, nativeVersion: '1.0.0' },
    export: { libName: [name] },
...
describe('getRebuildDeps', () => {
    const savedEnv = process.env.CROSSBIND_REBUILD_DEPS;

    afterEach(() => {
        if (savedEnv === undefined) delete process.env.CROSSBIND_REBUILD_DEPS;
        else process.env.CROSSBIND_REBUILD_DEPS = savedEnv;
    });
    ...
    test('comma list matches general.name, package.name and alias.package', () => {
        ...
        const byAlias = dep('zlib', { general: { name: 'zlib', alias: { package: '@crossbind/port-z' } } });
        const unmatched = dep('tiff');

        const result = getRebuildDeps(
            [byGeneral, byPackage, byAlias, unmatched],
            'gdal, @crossbind/port-proj, @crossbind/port-z',
        );
    ...
    test('falls back to CROSSBIND_REBUILD_DEPS env when rebuildOption is undefined', () => {
        process.env.CROSSBIND_REBUILD_DEPS = 'gdal';
...
describe('isCached / computeDependenciesStamp (fs-backed)', () => {
    ...
    beforeEach(() => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'crossbind-rebuild-'));
    });
    ...
    const writeMarker = (depsDir, key) => {
        ...
        fs.writeFileSync(`${depsDir}/.crossbind-rebuild.json`, JSON.stringify({ key }));
    };
