...
describe('guardAsyncBindings', () => {
    test('wraps a chained async registration in #ifdef CROSSBIND_JSPI', () => {
        const input = [
...
describe('getFilesFingerprint', () => {
    test('is order-insensitive and content-sensitive', () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'crossbind-fp-'));
    ...
    test('missing files fingerprint deterministically instead of throwing', () => {
        const ghost = path.join(os.tmpdir(), 'crossbind-fp-missing', 'ghost.cpp');
