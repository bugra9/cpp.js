...
// The pthread spawn script is handed to the glue via the crossbind-specific
// crossbindMainScript key (buildWasm rewrites the bootstrap to read it). Using
...
describe('fs-browser extendModule crossbindMainScript', () => {
    test('uses paths.worker when set', () => {
        const m = makeModule();
        fsBrowser.extendModule(m, { paths: { worker: 'my.worker.js' } });
        expect(m.crossbindMainScript).toBe('/my.worker.js');
    });

    test('falls back to paths.js', () => {
        const m = makeModule();
        fsBrowser.extendModule(m, { paths: { js: 'crossbind.js' } });
        expect(m.crossbindMainScript).toBe('/crossbind.js');
    });

    test('leaves the property unset when no script path is known', () => {
        const m = makeModule();
        fsBrowser.extendModule(m, { paths: {} });
        expect('crossbindMainScript' in m).toBe(false);
    });

    test('never sets emscripten-validated mainScriptUrlOrBlob', () => {
        const m = makeModule();
        fsBrowser.extendModule(m, { paths: { worker: 'my.worker.js', js: 'crossbind.js' } });
