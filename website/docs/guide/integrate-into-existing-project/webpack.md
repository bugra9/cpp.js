...
The Webpack plugin auto-injects COOP/COEP headers in the dev server, so `runtime: 'mt'` works in development out of the box. **Production hosting requires you to configure these headers yourself** (Vercel, Netlify, nginx, …) — see the [threading guide](/docs/api/configuration/threading) for per-host snippets.
:::
