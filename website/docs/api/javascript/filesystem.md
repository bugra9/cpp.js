...
4. **Using `Module.FS` before init completes** — `Module.FS` is undefined until `await initNative(...)` resolves. Use the `onRuntimeInitialized: (m) => { … }` hook or await the promise first.
