...
1. **Custom type mappings** (`%typemap` directives) — converting an exotic upstream type to a JS-friendly one.
2. **Selective symbol export** — your header has 100 symbols and you want to expose only 10. Auto-generated `.i` exposes everything.
...
If a `.i` file isn't enough either, the only remaining option is to **wrap in C++** — see the wrapper pattern in [C++ binding rules](/docs/api/cpp-bindings/overview). Anything reachable from a clean wrapper class with binding-friendly types will bind.

## See also

- [C++ binding rules](/docs/api/cpp-bindings/overview) — what auto-generated bindings can and can't handle.
- [Paths](/docs/api/configuration/paths) — `paths.module` and `ext.module` fields.
- [Troubleshooting](/docs/guide/troubleshooting) — SWIG generation failures.

:::info
**Source:** [createInterface.js](https://github.com/bugra9/cpp.js/blob/main/cppjs-core/cpp.js/src/actions/createInterface.js) — auto-generation logic.
:::
