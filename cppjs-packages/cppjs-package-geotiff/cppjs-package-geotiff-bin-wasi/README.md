# @cpp.js/package-geotiff-bin-wasi

The upstream **libgeotiff tools** (`listgeo`, `geotifcp`, `applygeo`), built by libgeotiff's own build system, shipped as **WASI command components** (`wasm32-wasip3`) with [cpp.js](https://cpp.js.org). Link against [`@cpp.js/package-geotiff-wasi`](https://www.npmjs.com/package/@cpp.js/package-geotiff-wasi) when you build your own tool, install this package when you just want to run the tools.
...
```bash
npm i -g @cpp.js/package-geotiff-bin-wasi
...
One-off use without installing globally: `npx -p @cpp.js/package-geotiff-bin-wasi listgeo-wasi image.tif`. Requires a WASI 0.3 runtime with Wasm 3.0 exception support (wasmtime 47+).
...
```bash
npm i @cpp.js/package-geotiff-bin-wasi
