# @cpp.js/package-zstd-bin-wasi

The upstream **`zstd` CLI**, built by zstd's own build system, shipped as a **WASI command component** (`wasm32-wasip3`) with [cpp.js](https://cpp.js.org). Link against [`@cpp.js/package-zstd-wasi`](https://www.npmjs.com/package/@cpp.js/package-zstd-wasi) when you build your own tool, install this package when you just want to run zstd.
...
```bash
npm i -g @cpp.js/package-zstd-bin-wasi

zstd-wasi file -o file.zst
zstd-wasi -d file.zst -o file.out
```

One-off use without installing globally: `npx -p @cpp.js/package-zstd-bin-wasi zstd-wasi -d file.zst -o file`. Requires a WASI 0.3 runtime with Wasm 3.0 exception support (wasmtime 47+).
...
```bash
npm i @cpp.js/package-zstd-bin-wasi
