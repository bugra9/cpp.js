# @cpp.js/package-geos-bin-wasi

The upstream **`geosop`** geometry operation CLI, built by GEOS's own build system, shipped as a **WASI command component** (`wasm32-wasip3`) with [cpp.js](https://cpp.js.org). Link against [`@cpp.js/package-geos-wasi`](https://www.npmjs.com/package/@cpp.js/package-geos-wasi) when you build your own tool, install this package when you just want to run geosop.
...
```bash
npm i -g @cpp.js/package-geos-bin-wasi

geosop-wasi -a "POLYGON((0 0,10 0,10 10,0 10,0 0))" \
            -b "POLYGON((5 5,15 5,15 15,5 15,5 5))" intersection
```

One-off use without installing globally: `npx -p @cpp.js/package-geos-bin-wasi geosop-wasi ...`. Requires a WASI 0.3 runtime with Wasm 3.0 exception support (wasmtime 47+).
...
```bash
npm i @cpp.js/package-geos-bin-wasi
...
`bin/geosop` exactly as GEOS's build produces it (`-DBUILD_GEOSOP=ON`), statically linked. Single-threaded. cpp.js's wasi runtime stubs (the dlopen family, `tmpfile`, plus `getsockname`/`getpeername` shadows) ride along in every executable link.
