...
export default {
    ...
    getBuildParams: (target, depPaths) => [
        ...
        '-DGDAL_USE_ZSTD=ON', `-DZSTD_INCLUDE_DIR=${depPaths.zstd.header}`, `-DZSTD_LIBRARY=${depPaths.zstd.lib}`,
        '-DGDAL_USE_LERC=ON', `-DLERC_INCLUDE_DIR=${depPaths.Lerc.header}`, `-DLERC_LIBRARY=${depPaths.Lerc.lib}`,
        ...
        '-DGDAL_USE_CURL=ON', `-DCURL_INCLUDE_DIR=${depPaths.curl.header}`, `-DCURL_LIBRARY=${depPaths.curl.lib}`,
    ],
    env: [
