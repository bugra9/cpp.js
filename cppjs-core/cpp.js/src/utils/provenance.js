...
import { getDockerImage } from './pullDockerImage.js';
...
export default function buildProvenance(target) {
    ...
    if (!family) {
        throw new Error(`cppjs: cannot resolve the family recipe package of "${manifest.name}" - required for provenance (contract K4).`);
    }
    const { getURL, sha256 } = state.config.build;
    if (!sha256) {
        throw new Error(`cppjs: the recipe of "${manifest.name}" declares no source sha256 - required for provenance (contract K4).`);
    }
