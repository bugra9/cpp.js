...
async function initProcessState() {
    ...
    // Same startup-file-map rule for the bare `cpp.js` runtime module: metro rewrites the
    // import to this placeholder, so on a fresh checkout it must exist before the crawl -
    // a lazy write inside resolveRequest lands after the file map and stays invisible.
    const runtimeFile = `${state.config.paths.cache}/cppjs-runtime.js`;
    if (!fs.existsSync(runtimeFile)) {
        fs.mkdirSync(path.dirname(runtimeFile), { recursive: true });
        fs.writeFileSync(runtimeFile, '// Generated by cpp.js - replaced per platform at transform time.\n');
    }
