...
describe('formatCycloneDxSbom', () => {
    ...
    test('emits deterministic CycloneDX with purl, hash, license expression', () => {
        const sbom = JSON.parse(formatCycloneDxSbom([row()], { name: '@cpp.js/package-zlib-wasi', version: '2.0.0' }));

        expect(sbom.bomFormat).toBe('CycloneDX');
        expect(sbom.metadata.component.name).toBe('@cpp.js/package-zlib-wasi');
