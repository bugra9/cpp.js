// jsi ADAPTER (mobile): stable flat C-ABI -> @cpp.js/core-embind-jsi.
//
...
// embind-jsi's TYPEID is `const void*` (wire.h) - identical to our CppjsTid, so no conversion.
...
void cppjs_embind_register_optional(const void* opt, const void* /*inner: adapter converts*/) {
...
extern "C" void cppjs_embind_raise_error(uint8_t* w) {
    uint32_t len;
    std::memcpy(&len, w, 4);
...
void cppjs_embind_register_class_constructor(
    CppjsTid cls, unsigned argc, const CppjsTid* argTypes,
...
void cppjs_embind_register_class_class_function(
    CppjsTid cls, const char* name, unsigned argc, const CppjsTid* argTypes,
    const char* sig, CppjsFn invoker, CppjsFn method, bool isAsync, bool /*isNonnullReturn dropped*/) {
...
void cppjs_embind_register_function(
    ...
    const char* sig, CppjsFn invoker, CppjsFn function, bool isAsync, bool /*isNonnullReturn dropped*/) {
...
void cppjs_embind_register_smart_ptr(
    CppjsTid ptr_ty, CppjsTid pointee_ty, const char* name, int sharing,
...
void cppjs_embind_register_enum(CppjsTid enum_ty, const char* name, size_t size, bool is_signed, int /*policy dropped*/) {
...
void cppjs_embind_register_value_object(CppjsTid struct_ty, const char* name,
...
void cppjs_embind_register_value_object_field(CppjsTid struct_ty, const char* field_name,
