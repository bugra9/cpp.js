# @cpp.js/core-embind-rust

Rust **producer** for the cpp.js embind registration protocol. Write an idiomatic Rust
struct, register it, and it surfaces in JS as a real class - on web through emscripten's
embind, on mobile through [`@cpp.js/core-embind-jsi`](../cppjs-core-embind-jsi) - **with no
...
Rust  (this package)   ──┼── cppjs_embind_register_* ─┤
                         │                            └──  mobile embind-jsi
...
- `include/cppjs_embind.h` - the stable flat C-ABI the crate binds to (raw function
...
cpp.js-specific. Exactly like C++ (where the `.h` is enough and the toolchain generates the
`.i.cpp` bridge), the engine parses the crate's pub surface and generates a **companion bridge
crate** at `<pkg>/.cppjs/bridge-crate/` that depends on the untouched user crate by path:
...
`<project>/.cppjs/rust-bridges/<name>/` (the user file is embedded via `#[path] mod user;` -
...
into the app lib (iOS: merged into the force_loaded react-native-cppjs.a; Android: whole-archive).

App-local surfaces can use UPSTREAM crates directly - the C++-style model where the app writes
its own thin surface over a linked library and no cpp.js package is involved. Declare the crates
...
`@cpp.js/package-*-rs` stays possible for JS-only consumers - it is four files
(package.json + cppjs.config.mjs + crate/Cargo.toml + crate/src/lib.rs) - but this repo
ships none.
...
`cppjs_keep_<libName>()` symbol, is built with `codegen-units = 1` (keep symbol and init-array
constructor share one object), and is linked lazily with that symbol pinned
(iOS `-Wl,-u,_cppjs_keep_<lib>`, Android `-Wl,--undefined=cppjs_keep_<lib>`) - the registration
...
with `-Wl,--export=cppjs_keep_<lib>` instead; and any Rust archive in the link makes `buildWasm`
compile `adapters/web.cpp` into the app wasm (it provides the flat `cppjs_embind_*` C-ABI - on
...
Generation rules (`cpp.js/src/utils/rustBridgeGen.js`, run by `buildCargo` before cargo):
...
| `export.bindings.vectors: [{ of: 'i32', name: '..' }]` in cppjs.config.mjs | `register_vector` |
...
| typed package import (`import { X } from '@cpp.js/embind-rust-demo'`) | ✅ GREEN on iOS, Android AND web (vite) — resolver + proxy module + generated d.ts; web verified by playwright on chromium/firefox/webkit |
...
- Errors (`Result::Err`): Rust cannot unwind across the C-ABI, so shims call
  `embind_rs::raise_err(msg)` → the adapter-owned `cppjs_embind_raise_error`. On wasm that import
...
3. Type universe: ~~primitives (i32, f64, bool, String, void)~~ ✅, ~~smart_ptr / `.create`~~ ✅
   ...
   `cppjs_embind_register_function`), ~~`impl Display` → toString()~~ ✅,
...
5. ~~Real device smoke via React Native.~~ ✅ iOS simulator AND Android emulator, full demo 23/23
   each (see status matrix). Still open: version discipline, `#[cppjs::bind]` proc-macro ergonomics.
...
(generator JS in `cpp.js`, `embind-rs`, `adapters/*.cpp`) moves on a single npm version axis -
...
- React Native apps get it transitively via `@cpp.js/plugin-react-native`;
- web apps via `@cpp.js/plugin-vite` / `@cpp.js/plugin-rollup`;
- standalone cargo packages declare `@cpp.js/core-embind-rust` in their devDependencies
  (see `demo/package.json`).

Generated bridge manifests carry a `# embind-rs from @cpp.js/core-embind-rust <version>`
stamp and a machine-local path into the resolved install; they are per-machine build
artifacts under `.cppjs/` and are regenerated on every build. A crates.io release of the
crate is a 1.0-era option for hand-written `bindings!` crates; the toolchain path stays
npm-resolved either way.

## Building as a cpp.js package

Rust is a first-class cpp.js package via the engine's `buildType: 'cargo'` (no per-project
script). A package declares `export: { type: 'cargo', libName: ['demo'], crate: '.' }`; cpp.js
runs `cargo build` on the host for each platform's triple (`cppjs build -p ios` →
aarch64-apple-ios + -sim), stages the `.a` as a normal prebuilt, and its usual pipeline
produces the xcframework / links it. `demo/cppjs.config.mjs` is the working example. (iOS is
