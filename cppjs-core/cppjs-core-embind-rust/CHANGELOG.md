...
- Initial release: the Rust binding layer for cpp.js. A pure-Rust producer crate
  (`embind-rs`) speaks a stable flat C ABI (`include/cppjs_embind.h`); per-host
...
- The engine does not depend on this package: apps (or the bundler plugins,
  which carry it as a dependency) declare it, and cpp.js resolves it from the
