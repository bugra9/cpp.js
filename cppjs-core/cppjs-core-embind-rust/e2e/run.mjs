...
sh('cd demo && node ../../cpp.js/src/utils/rustBridgeGen.js');
const bridgeLib = (triple) => `demo/.cppjs/bridge-crate/target/${triple}/release/libdemo_cppjs_bridge.a`;

// (1) WEB
sh('cargo build --release --target wasm32-unknown-emscripten -q --manifest-path demo/.cppjs/bridge-crate/Cargo.toml');
...
sh('cargo build --release -q --manifest-path demo/.cppjs/bridge-crate/Cargo.toml');
sh('clang++ -std=c++17 e2e/jsi-shape-check.cpp -Wl,-force_load,demo/.cppjs/bridge-crate/target/release/libdemo_cppjs_bridge.a -o e2e/jsi-shape-check');
