---
name: crossbind-runtime-api
description: 'Use this skill the moment the user asks about crossbind runtime/build configuration, C++ binding rules, or build troubleshooting — phrases like "what options does init accept", "how do I enable OPFS / persistent storage in crossbind", "useWorker", "is crossbind multithread", "runtime: mt vs st", "COOP COEP for crossbind", "what fields go in crossbind.config.js", "crossbind.build.js hooks", "what shape is state / target in build hooks", "crossbind binding rules / can I use raw pointers", "writing a C++ wrapper for crossbind", "manual SWIG .i file in crossbind", "crossbind override mechanism for emccFlags / cmake / env", "crossbind build error / linker error / out of memory", "crossbind performance defaults / tunable flags", "do I need m.delete in crossbind", "TypeScript types for crossbind", "mount file from input in crossbind", "crossbind filesystem in browser / node / cloudflare worker", "edge runtime crossbind limits", "cargo: import / cargoDependencies / Rust bindings in crossbind", "platform wasi / wasi build / -bin-wasi tools / crossbind licenses". Pull the matching reference doc into context before answering.'
---

# crossbind-runtime-api

Answer crossbind runtime / config questions from the canonical reference docs, never from training-data guesses. The reference covers four surfaces; pick the one matching the question.
...
| `crossbind.config.js` fields (consumer, build-time) | `docs/api/crossbind-config.md` | `config` |
| `crossbind.build.js` hooks (package author, build-time) | `docs/api/crossbind-build.md` | `build` |
...
| `platform: 'wasi'` command builds, `-bin-wasi` tools, `crossbind licenses` | `docs/api/wasi.md` | `wasi` |
...
- `https://github.com/crossbind/crossbind/blob/main/docs/api/<file>.md`
...
2. **Pull the matching doc** into context. Three ways:
   - **MCP**: call `crossbind_get_api_reference({ topic: 'init' })` (or any topic from the table above).
...
5. **`crossbind.config.js` is build-time only.** Putting `useWorker: true` in it does nothing — that's a runtime option for `init(opts)`. Catch this confusion early.
6. **Reach for the least invasive override.** When a default doesn't fit, the order is: target filter → `targetSpecs[].specs.*` → `crossbind.config.js env / functions` → `crossbind.build.js` hooks → `extensions[]` → `~/.crossbind.json`. See `overrides.md`.
...
2. **"How do I make crossbind multithreaded?"**
   → `target.runtime: 'mt'` in `crossbind.config.js` + COOP/COEP headers in production. See `threading.md`.
...
4. **"Can I use crossbind on Cloudflare Workers?"**
...
5. **"What fields go in `crossbind.config.js`?"**
   → `general.name`, `dependencies[]`, `paths.{config, project, native[], output, …}`, `target.runtime`, `export.{type, libName[]}`. See `crossbind-config.md` for the full shape with defaults.

## Don't

- Don't answer crossbind API questions from training-data assumptions. The API has evolved; load the current doc.
- Don't conflate `crossbind.config.js` (build-time) with `init(opts)` (runtime). Different surfaces, different fields.
- Don't conflate `useWorker: true` with `runtime: 'mt'`. They're orthogonal axes (see the matrix in `threading.md`).
- Don't suggest writing `crossbind.build.js` to a consumer — that file is package-author-only.
...
Full index: https://github.com/crossbind/crossbind/blob/main/docs/api/README.md
