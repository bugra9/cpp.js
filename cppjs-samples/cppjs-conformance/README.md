...
- `native/conformance.h` — header-only C++ surface (class/fields/methods, string/vector,
  ...
  by package subpath (`@cpp.js/conformance/native/conformance.h`); standalone builds list
  `../cppjs-conformance/native` in `paths.header`.
...
| Leg | App | Run |
|-----|-----|-----|
| node st (direct module) | `cppjs-playground-backend-nodejs` | `pnpm build && pnpm e2e:prod` |
| node mt (direct module, pthreads) | `cppjs-playground-backend-nodejs-multithread` | `pnpm build && pnpm e2e:prod` |
| browser ×3 (vite plugin, worker-backed) | `cppjs-playground-web-vite` | `pnpm build && playwright test --config playwright.prod.config.cjs` |
| browser ×3 (vite plugin, mt + worker) | `cppjs-playground-web-vite-multithread` | `pnpm build && playwright test --config playwright.prod.config.cjs` |
| browser ×3 (webpack plugin via rspack, mt, worker-backed) | `cppjs-playground-web-rspack` | `pnpm build && pnpm e2e:prod` |
| browser ×3 (no plugin, CLI standalone, mt) | `cppjs-playground-web-vanilla` | `pnpm build && pnpm e2e:prod` |
| edge (cloudflare worker, direct module) | `cppjs-playground-cloud-cloudflare-worker` | `pnpm build && playwright test --config playwright.dev.config.cjs` |
| Android + iOS (jsi) | `cppjs-playground-mobile-reactnative-cli` | `pnpm run:android` / `pnpm run:ios` — the screen prints `CONFORMANCE pass/run` |

Plugin coverage: the legs exercise every bundler plugin in the repo — vite st and mt (the
vite plugin wraps `@cpp.js/plugin-rollup`, so the rollup core rides along), webpack/rspack,
