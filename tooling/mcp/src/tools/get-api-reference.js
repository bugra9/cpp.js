...
import { z } from 'zod';
import { findCrossbindRoot } from '../repo-root.js';

export const name = 'crossbind_get_api_reference';

const TOPIC_TO_FILE = {
    index: 'README.md',
    init: 'init.md',
    config: 'crossbind-config.md',
    build: 'crossbind-build.md',
...
const GITHUB_BASE = 'https://github.com/crossbind/crossbind/blob/main/docs/api';

export const config = {
    title: 'Get crossbind API reference doc by topic',
    description: 'Return the canonical reference document for a crossbind API surface. Use BEFORE answering questions about init(opts), crossbind.config.js fields, crossbind.build.js hooks, OPFS / persistent storage / filesystem, threading (st vs mt), useWorker, COOP/COEP, or edge-runtime limits. The 13 topics map 1:1 to docs/api/*.md files. Reads the file from the local crossbind checkout when available; otherwise returns the GitHub URL the agent can fetch.',
    inputSchema: {
        topic: z
            .enum([
            ...
            .describe(
                ...
                + 'init = init(opts) runtime API; '
                + 'config = crossbind.config.js (consumer build-time); '
                + 'build = crossbind.build.js (package author only); '
                + 'filesystem = OPFS / memfs / node-fs / edge fs decision tree; '
                + 'threading = runtime st vs mt + useWorker + COOP/COEP + edge limits; '
                + 'binding-rules = rules for writing C++ that crossbind can auto-bind; '
                + 'swig = manual SWIG .i escape hatch; '
                + 'state = state and target object shapes for crossbind.build.js hooks; '
...
export async function handler({ topic }) {
    ...
    return {
        content: [
            {
                type: 'text',
                text: `Local docs/api/${fileName} not found (MCP not running inside a crossbind checkout). Fetch the canonical version from:\n${url}`,
            },
        ],
    };
}
