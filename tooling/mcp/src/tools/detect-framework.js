...
export const name = 'crossbind_detect_framework';

export const config = {
    title: 'Detect framework of a JS/TS project',
    description: 'Inspect a project directory and identify its bundler / runtime (vite, webpack, rspack, rollup, nextjs, react-native-cli, react-native-expo, cloudflare-worker, nodejs, vanilla). Returns a JSON match plus the recommended crossbind integration playbook URL. Wraps scripts/detect-framework.js from the crossbind repo.',
    inputSchema: {
        projectPath: z
            .string()
            .optional()
...
export async function handler({ projectPath }) {
    ...
    // Always run the bundled script from the crossbind checkout; a caller-supplied path would
    // let a prompt-injected agent execute an arbitrary file as node.
    const script = resolveDetectScript();

    if (!script) {
        return errorResponse(
            'Could not locate scripts/detect-framework.js. Either run the MCP server from inside a crossbind checkout '
