# AGENTS.md — @crossbind/mcp

You're editing the **MCP server** for crossbind. It exposes typed tools to MCP clients (Claude Desktop, Claude Code, Cursor, Codex, …) over stdio JSON-RPC.

## Layout

```
crossbind-mcp/
├── package.json            (npm: @crossbind/mcp, bin: crossbind-mcp)
├── bin/crossbind-mcp.js        (#!/usr/bin/env node shim → src/index.js)
├── src/
│   ├── index.js            (McpServer + StdioServerTransport, tool registration loop)
│   ├── repo-root.js        (find/require crossbind monorepo root via marker triple)
...
```js
export const name = 'crossbind_<tool_name>';   // snake_case, MCP-side identifier
export const config = {
    title: '...',
...
- **Monorepo-required tools call `requireCrossbindRoot()`.** It throws a clear error if cwd isn't inside a crossbind checkout. Project-facing tools call `findCrossbindRoot()` (returns null) and degrade gracefully.
- **Add long-running tools cautiously.** The default subprocess timeout is 10 min. `crossbind_build_port` overrides to 30 min. Don't go higher without justification — the MCP client will block on the call.
...
- Bind to internals of `core/crossbind/`. The MCP wraps **scripts** and **pnpm**, not source modules. This isolates us from refactors of the build pipeline.
...
- Pairs with the Claude Code plugin in `agents/`.
