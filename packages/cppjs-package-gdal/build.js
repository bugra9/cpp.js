...
import expatConfig from '@cpp.js/package-expat/cppjs.config.js';
import geosConfig from '@cpp.js/package-geos/cppjs.config.js';
import geotiffConfig from '@cpp.js/package-geotiff/cppjs.config.js';
import iconvConfig from '@cpp.js/package-iconv/cppjs.config.js';
import projConfig from '@cpp.js/package-proj/cppjs.config.js';
import spatialiteConfig from '@cpp.js/package-spatialite/cppjs.config.js';
...
import webpConfig from '@cpp.js/package-webp/cppjs.config.js';
...
compiler2.getAllPlatforms().forEach((platform) => {
    ...
    const job = async () => {
        ...
        const EMCC_CFLAGS = '-DRENAME_INTERNAL_LIBTIFF_SYMBOLS';
        compiler.run(null, [
        ...
        ], { workdir, console: true, params: [
            '-e', `CFLAGS="${EMCC_CFLAGS}"`,
            '-e', `CPPFLAGS="${EMCC_CFLAGS}"`,
            '-e', `EMCC_CFLAGS="${EMCC_CFLAGS}"`,
        ], });
        if (basePlatform === 'iOS') {
            compiler.run(null, ['ios-cmake', '--build', '.', '--config', 'Release', '--target', 'install'], { workdir, console: true });
        } else {
            compiler.run(null, ['make', `-j${cpuCount}`, 'install'], { workdir, console: true, params: [
                '-e', `CFLAGS="${EMCC_CFLAGS}"`,
                '-e', `CPPFLAGS="${EMCC_CFLAGS}"`,
                '-e', `EMCC_CFLAGS="${EMCC_CFLAGS}"`,
            ], });
