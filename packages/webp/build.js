...
await decompress(`${compiler.config.paths.temp}/libwebp-${VERSION}.tar.gz`, compiler.config.paths.temp, { plugins: [decompressTargz()] });
...
compiler.run('emconfigure', ['./configure', `--prefix=/live/${libdir}`, '--enable-shared=no', '--host=wasm32-unknown-emscripten'], { workdir, console: true });
